...
def crf_proc(config):
    ...
    def _job(i):
        ...
        name = name_list[i]
        logit_name = os.path.join(args.work_dir, "logit", name[13:] + ".npy")
...
def main(cfg):
    ...
    gts, preds, msc_preds, cams, preds_hist, msc_preds_hist, cams_hist = validate(model=model, dataset=val_dataset, test_scales=[1, 1.5])
