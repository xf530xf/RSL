"""RSL model for the CUB-200-2011 weakly-supervised localization dataset.

Clone of the active VOC / underwater model (see ``model_attn_aff_voc.py`` and
``model_attn_aff_underwater.py``) with two changes:
  - text prompts come from ``clip.cub_text`` (200 bird species + nature-scene bg);
  - per-image class ids come in via ``cls_labels`` and are routed into
    ``perform_single_cub_cam`` (no Pascal-VOC XML annotations).
...
from clip.clip_tool import (
    generate_cam_label,
    generate_clip_fts,
    perform_single_cub_cam,
)
from clip.cub_text import (
...
def _cls_label_to_ids(cls_label):
    ...
    foreground class ids in model space (0 = bg, 1..200 = species)."""
...
class RSL(nn.Module):
    ...
    def classify(self, img):
        """Zero-shot CLIP classification over the 200 CUB species.

        Reuses the model's own CLIP image-embedding path (``encode_image`` +
        ``forward_last_layer``) against the 200 foreground text features.
        Returns a ``(B, 200)`` tensor of class probabilities; column ``j``
        maps to ``class_names_cub[j]`` (model-space species id ``j + 1``).
        """
        b, c, h, w = img.shape
        image_features, _ = self.encoder.encode_image(
            img, h, w, require_all_fts=False, clip_flag=self.clip_flag,
        )
        # Route the last CLIP block through the GradCAM plumbing so its forward
        # hook gets the right token grid (height/width) and the activation lists
        # are reset — exactly like the CAM path does.
        logits_per_image, _ = self.grad_cam.activations_and_grads(
            [image_features, self.fg_text_features], h, w,
        )
    ...
    def forward(self, img, img_names, cls_labels=None, mode='train'):
        ...
        for i, img_name in enumerate(img_names):
            ...
            cam_refined_list, keys, w_out, h_out = perform_single_cub_cam(
            ...
            cam_raw_list.append(cam_dict['refined_cam'].detach().cpu())
        ...
        return seg_sam3, seg_dino, all_cam_labels, attn_pred, cam_raw_list
