"""RSL VOC model with a DINOv2 encoder (separate from the DINOv3/timm version).

This file mirrors ``model_attn_aff_voc.py`` but swaps the DINO branch from
the timm-based DINOv3 backbone to a ``torch.hub`` DINOv2 backbone.

Key differences vs. the DINOv3 version:
  * DINOv2 is loaded with ``torch.hub.load('facebookresearch/dinov2', dino_model)``.
  * DINOv2 uses a patch size of 14 (DINOv3/timm here used 16).
  * Patch tokens are read directly from ``forward_features(img)['x_norm_patchtokens']``
    (no CLS/register prefix tokens to skip).

Everything else (SAM3 branch, Text-Conditioned Modulation, decoder, CAM
pipeline, returned tensors) is kept identical so the existing training /
loss / EMA code works unchanged.
"""
...
class RSL(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, sam3_model='facebook/sam3',
                 dino_model='dinov2_vitl14', dino_fts_dim=1024, decoder_layers=5,
                 embedding_dim=256, in_channels=None, dataset_root_path=None,
                 clip_flag=16, dino_patch_size=14, device='cuda'):
        super().__init__()
        ...
        # DINOv2 encoder (loaded from torch.hub, frozen)
