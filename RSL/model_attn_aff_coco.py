...
class RSL(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, dino_model=None, dino_fts_dim=768, decoder_layers=3,
                 embedding_dim=256, in_channels=512, dataset_root_path=None, clip_flag=16, dino_patch_size=16, device='cuda'):
        super().__init__()
    ...
    def forward_seg(self, img):
        ...
        fts_all, _ = generate_clip_fts(img, self.encoder, require_all_fts=True, clip_flag=self.clip_flag)
    ...
    def forward(self, img, img_names='2007_000032', mode='train'):
        ...
        # Generate CAMs for each image (both train and val modes)
        for i, img_name in enumerate(img_names):
            img_folder = 'train' if mode == 'train' else 'val'
            img_path = os.path.join(self.root_path, img_folder, str(img_name) + '.png')
            img_i = img[i]
            ...
            cam_refined_list, keys, w, h = perform_single_coco_cam(
        ...
        # For training we return detailed outputs; for eval we return fused segs, cams and attn
        if self.training:
            return seg_clip, seg_dino, all_cam_labels, attn_pred, fts, dino_fts
        else:
            fused_segs = 0.5 * seg_clip + 0.5 * seg_dino
            return fused_segs, all_cam_labels, attn_pred
