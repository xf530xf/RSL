...
class RSL(nn.Module):
    def __init__(self, num_classes=None, clip_model=None, dino_model=None, dino_fts_dim=768, decoder_layers=3,
        ...
        self.dino_encoder = torch.hub.load('facebookresearch/dinov3', dino_model)
    ...
    def forward(self, img, img_names='2007_000032', mode='train'):
        ...
        if isinstance(dino_fts, list):
            for d_i, dino_fts_single in enumerate(dino_fts):
                dino_fts_single = dino_fts_single.reshape([b, dino_img_h // ps, dino_img_w // ps, -1]).permute(0, 3, 1,
                                                                                                               2)
            ...
            dino_fts = dino_fts.reshape([b, dino_img_h // ps, dino_img_w // ps, -1]).permute(0, 3, 1, 2)
...
if __name__=="__main__":
    ...
    model = RSL('mit_b1', num_classes=20, embedding_dim=256, pretrained=True)
    model._param_groups()
