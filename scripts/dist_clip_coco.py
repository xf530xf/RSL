...
from RSL.model_attn_aff_coco import RSL
...
def validate(model=None, data_loader=None, cfg=None):
    ...
    for _, data in tqdm(enumerate(data_loader),
        ...
        if num % 1000 == 0:
            ...
            cam_hist, cam_score = evaluate.scores(gts, cams, cam_hist, num_classes=81)
            preds, gts, cams, aff_gts = [], [], [], []
...
def train(cfg):
    ...
    ema_replace_start = int(getattr(cfg.train, "ema_replace_start", 20000))
    ema_ramp_iters = int(getattr(cfg.train, "ema_ramp_iters", 40000))
