"""Training script for the AquaOV255 underwater pipeline.

Mirrors `scripts/dist_clip_voc.py`: same EMA-teacher + prototype + boundary +
dice + attn-affinity losses, but uses the underwater dataset module / model and
forwards per-sample `cls_label` tensors into the model so CAM derivation can
run without Pascal-VOC XML annotations.
"""
...
parser.add_argument(
    '--config',
    default='configs/underwater_attn_reg.yaml',
...
def validate(model=None, data_loader=None, cfg=None):
    ...
    preds, gts, cams = [], [], []
    num = 1
    seg_hist = np.zeros((num_classes, num_classes))
    cam_hist = np.zeros((num_classes, num_classes))
    ...
    for _, data in tqdm(enumerate(data_loader),
        ...
        if not (h % flag == 0 and w % flag == 0):
            new_h = round(h / flag) * flag
            new_w = round(w / flag) * flag
            inputs = F.interpolate(inputs, size=(new_h, new_w), mode='bilinear', align_corners=False)

        segs_clip, segs_dino, cam, attn_pred = model(inputs, name, cls_labels=cls_label, mode='val')
        ...
        if num % 1000 == 0:
            ...
            cam_hist, _ = evaluate.scores(gts, cams, cam_hist, num_classes=num_classes)
            preds, gts, cams = [], [], []

    seg_hist, seg_score = evaluate.scores(gts, preds, seg_hist, num_classes=num_classes)
    cam_hist, cam_score = evaluate.scores(gts, cams, cam_hist, num_classes=num_classes)
    model.train()
...
def train(cfg):
    ...
    train_dataset = underwater.UnderwaterClsDataset(
