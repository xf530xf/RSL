"""Training script for the CUB-200-2011 weakly-supervised localization pipeline.

Mirrors `scripts/train-underwater.py` (same EMA-teacher + prototype + boundary +
dice + attn-affinity losses), but uses the CUB dataset module / model and
evaluates **object localization** (GT-Known Loc / mean-IoU against CUB bounding
boxes) instead of segmentation mIoU, since CUB ships no pixel masks.
"""
...
parser.add_argument(
    '--config',
    default='configs/cub_attn_reg.yaml',
...
def _foreground_map_from_segs(segs, size_hw):
    """segs: (1, C, h, w) logits -> foreground score map at original (H, W)."""
    seg_up = F.interpolate(segs, size=size_hw, mode='bilinear', align_corners=False)
    prob = F.softmax(seg_up, dim=1)[0]
    fg = 1.0 - prob[0]  # 1 - p(background)
    return fg.detach().cpu().numpy()


def _foreground_map_from_cam(cam_label):
    """cam_label: (H, W) integer pseudo-label map (0 = bg) -> binary fg map."""
    return (cam_label != 0).astype(np.float32)


def validate_loc(model=None, data_loader=None, cfg=None):
    # NOTE: no torch.no_grad() here — the model's internal CLIP GradCAM needs
    # gradients.  Seg outputs are detached before any numpy conversion below.
    model.eval()
    ...
    for _, data in tqdm(enumerate(data_loader),
        ...
        segs_clip, segs_dino, cam, _ = model(inputs, name, cls_labels=cls_label, mode='val')
        segs = 0.5 * (segs_clip + segs_dino)

        gt = np.asarray(gt_box[0].numpy(), dtype=np.float32)

        fg_seg = normalize_scoremap(_foreground_map_from_segs(segs, (H, W)))
        ...
        if cam_map.shape != (H, W):
            fg_cam = _foreground_map_from_cam(cam_map)
            fg_cam = np.asarray(
                F.interpolate(
                    torch.from_numpy(fg_cam)[None, None], size=(H, W), mode='nearest'
                )[0, 0]
            )
        else:
            fg_cam = _foreground_map_from_cam(cam_map)
        ...
        seg_iou_sum += seg_iou
        cam_iou_sum += cam_iou
        seg_hit += int(seg_iou >= iou_thr)
        cam_hit += int(cam_iou >= iou_thr)
        n_total += 1

    model.train()
    n_total = max(1, n_total)
    return {
        'seg_loc_acc': seg_hit / n_total,
        'cam_loc_acc': cam_hit / n_total,
        'seg_mean_iou': seg_iou_sum / n_total,
        'cam_mean_iou': cam_iou_sum / n_total,
    }
...
def get_uncertainty_confidence(seg_a, seg_b):
    ...
    skl = 0.5 * (kl_ab + kl_ba)
    disagreement = 1.0 - torch.exp(-skl)
...
def train(cfg):
    ...
    train_dataset = cub.CUBClsDataset(
    ...
    optimizer = PolyWarmupAdamW(
        params=[
            ...
            {'params': param_groups[1], 'lr': 0.0, 'weight_decay': 0.0},
            {'params': param_groups[2], 'lr': cfg.optimizer.learning_rate * 10,
             'weight_decay': cfg.optimizer.weight_decay},
            {'params': param_groups[3], 'lr': cfg.optimizer.learning_rate * 10,
             'weight_decay': cfg.optimizer.weight_decay},
        ],
    ...
    for n_iter in range(cfg.train.max_iters):
        ...
        segs_clip, segs_dino, cam, attn_pred, fts, dino_fts = model(
            inputs.cuda(), img_name, cls_labels=cls_labels,
        )
        ...
        aff_label = cams_to_affinity_label(
            fts_cam, mask=attn_mask, ignore_index=cfg.dataset.ignore_index,
            clip_flag=cfg.clip_init.clip_flag,
        )
        ...
        seg_clip_loss2 = get_seg_loss(segs_clip, pred_label_dino.type(torch.long),
                                      ignore_index=cfg.dataset.ignore_index)
        seg_dino_loss2 = get_seg_loss(segs_dino, pred_label_clip.type(torch.long),
                                      ignore_index=cfg.dataset.ignore_index)
        ...
        if (n_iter + 1) % cfg.train.eval_iters == 0:
            ...
            loc_score = validate_loc(model=model, data_loader=val_loader, cfg=cfg)
            logging.info('localization score:')
            logging.info(loc_score)
            writer.add_scalars('val/loc', loc_score, global_step=n_iter)
