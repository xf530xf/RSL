...
from clip.underwater_text import class_names_underwater
from clip.cub_text import class_names_cub
...
def perform_single_voc_cam_sam(img_path, image, image_features, attn_weight_list, seg_attn, bg_text_features,
                               fg_text_features, cam, mode='train', require_seg_trans=False, seg_dino_cam=None,
                               clip_flag=16):
    """SAM-feature CAM generation with GradCAM + text prompts.

    `image_features` is expected to be a 4D tensor `(1, C, Hf, Wf)`.
    `attn_weight_list` can be `None` or a tensor/list with shape `(L, N, N)`.
    """
    bg_text_features = bg_text_features.cuda()
    fg_text_features = fg_text_features.cuda()
    ...
    bg_features_temp = bg_text_features.cuda()
    fg_features_temp = fg_text_features[label_id_list].cuda()
    text_features_temp = torch.cat([fg_features_temp, bg_features_temp], dim=0)
    input_tensor = [image_features, text_features_temp.cuda(), h, w]
    ...
    for idx, label in enumerate(label_list):
        ...
        keys.append(label_index)
        targets = [ClipOutputTarget(label_list.index(label))]
        grayscale_cam, _, attn_weight_last = cam(input_tensor=input_tensor, targets=targets, target_size=None)
        grayscale_cam = grayscale_cam[0, :]

        if idx == 0:
            if isinstance(attn_weight_list, list):
                if len(attn_weight_list) > 0:
                    attn_weight = torch.stack(attn_weight_list, dim=0)
                else:
                    attn_weight = attn_weight_last.unsqueeze(0)
            elif attn_weight_list is None:
                attn_weight = attn_weight_last.unsqueeze(0)
            else:
                attn_weight = attn_weight_list
                if attn_weight.dim() == 2:
                    attn_weight = attn_weight.unsqueeze(0)

            attn_weight = torch.cat([attn_weight, attn_weight_last.unsqueeze(0)], dim=0)
            ...
            if require_seg_trans:
                ...
                attn_weight = torch.sum(attn_mask * attn_weight, dim=0) / (torch.sum(attn_mask, dim=0) + 1e-5)
                attn_weight = attn_weight.detach() * seg_attn.squeeze(0).detach()
            else:
                attn_weight = torch.mean(attn_weight, dim=0).detach()

            _trans_mat = compute_trans_mat(attn_weight).float()

        box, cnt = scoremap2bbox(scoremap=grayscale_cam, threshold=0.4, multi_contour_eval=True)
        aff_mask = torch.zeros((grayscale_cam.shape[0], grayscale_cam.shape[1]), device=image_features.device)
        for i_ in range(cnt):
            x0_, y0_, x1_, y1_ = box[i_]
            aff_mask[y0_:y1_, x0_:x1_] = 1

        cam_to_refine = torch.as_tensor(grayscale_cam, device=image_features.device, dtype=torch.float32)
        if seg_dino_cam is not None:
            seg_dino_cam_per_class = seg_dino_cam[label_index + 1]
            seg_dino_cam_per_class = seg_dino_cam_per_class.unsqueeze(0).unsqueeze(0)
            seg_dino_cam_per_class = torch.squeeze(seg_dino_cam_per_class)
            cam_to_refine = cam_to_refine * seg_dino_cam_per_class
            aff_mask = torch.ones_like(aff_mask)

        aff_mask = aff_mask.view(1, grayscale_cam.shape[0] * grayscale_cam.shape[1])
        trans_mat = _trans_mat * aff_mask

        cam_to_refine = cam_to_refine.view(-1, 1)
        cam_refined = torch.matmul(trans_mat, cam_to_refine).reshape(feat_h, feat_w)
        cam_refined_list.append(cam_refined)
...
def perform_single_underwater_cam(img_path, image, image_features, attn_weight_list, seg_attn,
                                  bg_text_features, fg_text_features, cam, cls_label_ids,
                                  mode='train', require_seg_trans=False, seg_dino_cam=None,
                                  clip_flag=16):
    """CAM generation for the AquaOV255 underwater pipeline.

    Differences vs ``perform_single_voc_cam``:
      - per-image class ids are passed in (``cls_label_ids``) instead of being
        parsed from a Pascal-VOC XML annotation;
      - text prompts come from ``class_names_underwater``;
      - the returned ``keys`` are 0-indexed positions into
        ``class_names_underwater`` (so the model's ``valid_key = pad(keys+1)``
        idiom still aligns with the 1..254 fg slots in the seg head).

    ``cls_label_ids`` is expected to be a Python list / 1-D iterable of
    *model-space* (1-indexed) foreground class ids in ``[1, 254]``.
    """
    bg_text_features = bg_text_features.cuda()
    fg_text_features = fg_text_features.cuda()
    ...
    label_index_list = []
    for cid in cls_label_ids:
        cid = int(cid)
        if cid <= 0:
            continue
        zero_idx = cid - 1
        if 0 <= zero_idx < len(class_names_underwater) and zero_idx not in label_index_list:
            label_index_list.append(zero_idx)
    if len(label_index_list) == 0:
        label_index_list = [0]

    label_list = [class_names_underwater[i] for i in label_index_list]
    ...
    bg_features_temp = bg_text_features.cuda()
    fg_features_temp = fg_text_features[label_index_list].cuda()
    text_features_temp = torch.cat([fg_features_temp, bg_features_temp], dim=0)
    input_tensor = [image_features, text_features_temp.cuda(), h, w]

    _trans_mat = None
    for idx, label in enumerate(label_list):
        label_index = label_index_list[idx]
        keys.append(label_index)
        ...
        if idx == 0:
            if require_seg_trans is True:
                attn_weight = torch.cat([attn_weight_list, attn_weight_last], dim=0)
                if clip_flag == 16:
                    attn_weight = attn_weight[:, 1:, 1:][-6:]
                if clip_flag == 14:
                    attn_weight = attn_weight[:, 1:, 1:][-12:]
...
def perform_single_cub_cam(img_path, image, image_features, attn_weight_list, seg_attn,
                           bg_text_features, fg_text_features, cam, cls_label_ids,
                           mode='train', require_seg_trans=False, seg_dino_cam=None,
                           clip_flag=16):
    """CAM generation for the CUB-200-2011 WSOL pipeline.

    Mirrors ``perform_single_underwater_cam`` exactly, but resolves species
    names against ``class_names_cub`` (200 entries).  ``cls_label_ids`` is a
    list of *model-space* (1-indexed) foreground class ids in ``[1, 200]``;
    returned ``keys`` are 0-indexed positions into ``class_names_cub`` so the
    model's ``valid_key = pad(keys + 1)`` idiom keeps aligning with the 1..200
    fg slots in the seg head.
    """
    bg_text_features = bg_text_features.cuda()
    fg_text_features = fg_text_features.cuda()
    ...
    for cid in cls_label_ids:
        ...
        if cid <= 0:
            continue
        zero_idx = cid - 1
        if 0 <= zero_idx < len(class_names_cub) and zero_idx not in label_index_list:
            label_index_list.append(zero_idx)
    if len(label_index_list) == 0:
        label_index_list = [0]

    label_list = [class_names_cub[i] for i in label_index_list]
